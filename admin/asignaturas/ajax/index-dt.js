...
const createOrEdit = async (id = null) => {
  ...
  const title = isEditing ? "Editar unidad didáctica" : "Crear unidad didáctica";
  ...
  const result = await Swal.fire({
    ...
    html: `
      <input type="text" id="nombre" class="swal2-input" value="${data.nombre || ''}" placeholder="Nombre de unidad didática">
      <input type="text" id="descripcion" class="swal2-input" value="${data.descripcion || ''}" placeholder="Descripción de la unidad didáctica">
    `,
    focusConfirm: false,
  ...
  if (result.isConfirmed) {
    ...
    const successMessage = {
      ...
      text: `Unidad didáctica ${isEditing ? 'actualizada' : 'creada'}`,
...
const ver = async (id) => {
  try {
    ...
    console.error('Error al obtener detalles de la unidad didáctica:', error);
    Swal.fire("Error", "No se pudo obtener los detalles de la unidad didáctica", "error");
  }
};
